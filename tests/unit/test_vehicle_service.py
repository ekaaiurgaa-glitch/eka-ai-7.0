...
async def test_list_vehicles(db_session):
    await service.create_vehicle(
        db_session,
        schema.VehicleCreate(plate_number="V1", make="M1", model="M1", year=2020, fuel_type=schema.FuelType.petrol),
        "test_tenant",
    )
    await service.create_vehicle(
        db_session,
        schema.VehicleCreate(plate_number="V2", make="M2", model="M2", year=2021, fuel_type=schema.FuelType.diesel),
        "test_tenant",
    )
...
async def test_update_vehicle(db_session):
    vehicle = await service.create_vehicle(
        db_session,
        schema.VehicleCreate(plate_number="UP01AB1234", make="Tata", model="Nexon", year=2022, fuel_type=schema.FuelType.electric),
        "test_tenant",
    )
